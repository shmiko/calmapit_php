...
jQuery.noConflict(); jQuery(document).ready(function(){
	...
	var mainBox2 = jQuery('#main2'),
    	pageBox2 = jQuery(".pageContent"),
    	loading = jQuery('#loading'),
    	//iPad,iPhone,iPod...
    	deviceAgent = navigator.userAgent.toLowerCase(),
    	iPadiPhone = deviceAgent.match(/(iphone|ipod|ipad)/),
	...
	//ACCORDION TOGGLES	
		...
		jQuery(this).stop(true,true).animate({paddingLeft:"10px",backgroundColor:'#73FFB9', color:'#000'},300);
	},function(){
		jQuery(this).stop(true,true).animate({paddingLeft:"8px",backgroundColor:'#333',color:'#fff'},300);
	});
	
	//ACCORDION TOGGLES	
		jQuery('.toggleButton2').click(function(){
		jQuery(".toggleButton2").not(this).removeClass('opened2').next().slideUp(400);
		jQuery(".toggleButton2").not(this).children('span').html("+");
		jQuery(this).toggleClass('opened2').next().slideToggle(400);
		jQuery('.opened2').children('span').html("&times;");
		jQuery(this).not('.opened2').children('span').html("+");
		jQuery("html,body").animate({scrollTop:0},400);
		jQuery('body.page2 .entry2').slideToggle(400);
	}).hover(function(){
		jQuery(this).stop(true,true).animate({paddingLeft:"10px",backgroundColor:'#73FFB9', color:'#000'},300);
	},function(){
		jQuery(this).stop(true,true).animate({paddingLeft:"8px",backgroundColor:'#333',color:'#fff'},300);
    ...
    pageBox.live('click', function(){
	 ...
	 //CLOSE MAIN DIV
    jQuery("#closeBox2").live('click', function(){
    	mainBox2.fadeOut(200);
    	pageBox2.animate({top:"0px"},300);
    	return false;
    }); 
    
    //OPEN MAIN DIV
    pageBox2.live('click', function(){
    	jQuery(this).animate({top:"0px"},300);
    	mainBox2.fadeIn(200);
    	return false;
    }); 
	...
	if (iPadiPhone) {
		function windowSizes(){
			...
			if(mainBox2.outerHeight() > mainHeight) {	
				jQuery(mainBox2).css({height:mainHeight,overflow:"auto"});
			}
		}
	...
	//IF NOT iPad
		...
		//ADD HANDLE AND MAKE DRAGGABLE
		mainBox2.draggable({ handle:"#handle2",opacity: 0.8}).resizable();
		
		mainBox2.prepend("<div id='moveNotice2'></div>");
		
		jQuery("#handle2").hover(function(){		
			jQuery("#moveNotice2").stop(true,true).fadeIn(200);
		},function(){
			jQuery("#moveNotice2").stop(true,true).fadeOut(200);
		});
	}
