		...
		<table width="100%" cellspacing="0" cellpadding="0" class="inline-table">
			<tbody>
				...
				<tr>
					...
					<td>
						<?php foreach(explode(',',$group['channels']) as $channel_id){
							echo '<div class="nss-group-channel nss-icon-'.$nss->getChannelField($channel_id,'type').'">'.$nss->getChannelField($channel_id,'id').'</div>';
						} ?>
					</td>
			...
			<div class='nss-admin-form-row'>
				...
				<div class="field-area">
					...
					<select class="select-group-channel">
						...
						<?php foreach ($nss->get('channel_list') as $channelArray => $channel){
								$type = $channel['type']; 
								echo "<option value='".$channel['id'].' | '.$channel['type']."'>".$channel['id'].' | '.$channel['type']."</option>";
						} ?>
					</select>
				</div>		
			</div>
