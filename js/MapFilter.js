...
(function (){
	...
	EventClass.prototype.init = function ( params ) {
		...
		return this;
		//console.log( "this.dateStartObj is ready!", this.dateStartObj );
	};
	EventClass.prototype.getCoordsStr = function(){
		return this.validCoords ?  this.lt + "," + this.lg : '';
		//debug.log("********************************this.validCoords",this.validCoords);
	};
	EventClass.prototype.settitle = function ( ) {
		return this.title;
			
		//this.gCalid = eventID;
		//console.log("*******************sdfsdfdfsdfsdf*************2nd this.title,this.image",this.title,this.image);
	};
	...
	EventClass.prototype.insideCurMap = function(mapbox){
		...
		//debug.log("********************************2nd this.validCoords",this.validCoords);
	};
	...
	MarkersClass.prototype.getMarkerObj = function(coordsStrOrEvent){
		...
		if (typeof coordsStrOrEvent === 'string') {
			...
			//console.log( "this.allMarkers[cnMF.eventList[coordsStrOrEvent].getCoordsStr()] is ready!", this.allMarkers[cnMF.eventList[coordsStrOrEvent].getCoordsStr()] );
		} else {
	...
	MarkersClass.prototype.showEvent = function(eventObj){
		...
		for (var ii=0; ii < markerObj.eventList.length; ii++) {
			var sText = markerObj.eventList[ii];
			var sTemp = sText.Name;
			//console.log("QQQQQQQQQ sTemp", sTemp );
			if (markerObj.eventList[ii] === eventObj.id) {
				// found it, so no need to add
				//console.log("url and location", eventObj.url,eventObj.location );
				//'console.log("-=-=-=-=-=-=-=markerObj.eventList[ii] , eventObj.id,eventObj.image", markerObj.eventList[ii] ,eventObj.id,eventObj.image);
				return;
			}
		}
	...
	MarkersClass.prototype.createMarkerObj = function(eventObj){
		...
		//var getCalidStr = eventObj.getCalid();
	...
	MarkersClass.prototype.hideEvent = function(eventObj){
		...
		for (var ii=0; ii < markerObj.eventList.length; ii++) {
			var kkk = cnMF.eventList[i];
			var calname = kkk.name;
			var calendarTitle = kkk.description;
			//console.log( "Calendar title and name", calendarTitle,calname );
			if (markerObj.eventList[ii] === eventObj.id) {
				// found it, remove
	...
	//If gcTitle == "USA" 	{
	//	var image =  new google.maps.MarkerImage('./img/CheersMemoriesLogoTestSm.png',
	//																		new google.maps.Size(25, 34),
	//																		new google.maps.Point(0, 0)
	//																	); 
	//	} else 	{
	//				var image =  new google.maps.MarkerImage('./img/shmikmapicon.png',
	//																		new google.maps.Size(25, 34),
	//																		new google.maps.Point(0, 0)
	//																	);
	//				}
	...
	//console.log("cnMF", typeof (cnMF), cnMF);
	...
	cnMF.init = function (coreOptions) {
		...
		//console.log( "cnMF.curStartDay ready!", cnMF.curStartDay );
		cnMF.curEndDay	= coreOptions.oEndDay;
		//console.log( "cnMF.curEndDay ready!", cnMF.curEndDay );
		cnMF.origStartDay = coreOptions.oStartDay; //2014-08-11
		//console.log( "cnMF.origStartDay ready!", cnMF.origStartDay );
		cnMF.origEndDay   = coreOptions.oEndDay; //returns 872
		//console.log( "cnMF.origEndDay  ready!", cnMF.origEndDay  );
		...
		cnMF.tz.offset = coreOptions.tzOffset ? coreOptions.tzOffset : ''; // Offset in hours and minutes from UTC returns +10:00
		cnMF.tz.name = coreOptions.tzName ? coreOptions.tzName : 'unknown'; // Olson database timezone key (ex: Europe/Berlin) returns Australia/Sydney
		cnMF.tz.dst = coreOptions.tzDst ? coreOptions.tzDst : 'unknown'; // bool for whether the tz uses daylight saving time returns true
		cnMF.tz.computedFromBrowser = (cnMF.tz.name != 'unknown'); //returns nothing
		
		cnMF.myGeo = cnMF.geocodeManager();
		//console.log( "cnMF.coreOptions and coreOptions is an object - ready!", typeof(cnMF.coreOptions), cnMF.coreOptions );
		//coreOptions has such items as cbClosedInfoWindow,cbMapRedraw,cbMarkerClicked,cbOpenedInfoWindow,gmap object, oEndDay,oStartDay,tzDst,tzName,tzOffset
	}
	...
	cnMF.countKnownAddresses = function () {
		var xx = 0;
			...
			//for (var i = 0; i < entries.length; i++) //{
				var calendarEntry = cnMF.eventList[ii];
				var calendarTitle = calendarEntry.name;
				//console.log( "Calendar title ",calendarTitle);
			 // }
			if (cnMF.eventList[ii].validCoords) xx++;
			//console.log( "cnMF.eventList[ii].validCoords", cnMF.eventList[ii].validCoords );
		}
	
		F.reportData.knownAddr = xx;
		return xx;
	}
	...
	cnMF.addEvent = function(params){ // old addEventObj
		...
		e.settitle(cnMF.eventList.length);
	...
	cnMF.mapAllEvents = function(){
		...
		for (var i in cnMF.eventList) {
			...
			var guestList= kk.getGuestList();    
				for(var d=0; guestList!=null && d<guestList.length; d++){
				  console.log("Guest "+d+": "+guestList[d].getEmail()+", Status: "+guestList[d].getGuestStatus());      
				}
			//var calendarEntry = kk.length;
				//for (var ii=0; ii < cnMF.eventList.length; ii++) {
					//var kkk = cnMF.eventList[i];
					//console.log( 'kk', kk);	
				//var guests = kk.attendees.email;
				//console.log( 'guests', guests);	
				//console.log( "guests", guests );	
				//for (var ii=0; ii < guests.length; ii++) {	
				//var calendarEntry = guests[0].email;
				//}
				//console.log( 'calendarEntry', calendarEntry);	
			//for (var ii=0; ii < kk.length; ii++) {
				//for (var i = 0; i < entries.length; i++) {
			//	var calendarEntry = kk.title;
				//for (var ii=0; ii < cnMF.eventList.length; ii++) {
					//var kkk = cnMF.eventList[i];
			//	console.log( 'calendarEntry', calendarEntry);	
				//}
	...
	cnMF.processGeocode = function(gObj) {
		for (var ii in cnMF.eventList) {
			...
			//console.log( 'received duplicate geocode', gObj);
			...
			//console.log("AAA   gObj.image,kk.image", gObj.image,kk.image);
		}
	...
	cnMF.updateMarkers= function(){
		...
		for (var i in cnMF.eventList) {
			var kk = cnMF.eventList[i];
			//debug.log( "kk.validCoords",kk.validCoords);
	...
	cnMF.getGCalData = function(gCalUrl, startDays, endDays, callbacks ) {
		...
		startDate = new Date();//new Date("March 10, 2016 00:00:00");//new Date();"March 10, 2016 11:13:00"
		startDate.setTime(startDate.getTime() + startDays*24*3600*1000);
		//console.log( "startDate ready!", startDate );
		startmin = cnMF.rfc3339(startDate,false);
		//console.log( "startmin ready!", startmin );
		debug.debug(" getGCalData(): start-min: "+startmin);
		//debug.log('getGCalData(): start-min: '+startmin);
		endDate = new Date("March 10, 2016 00:00:00");//new Date();"March 10, 2016 11:13:00"
		endDate.setTime(endDate.getTime() + endDays*24*3600*1000);
		//console.log( "endDate ready!", endDate );
		startmax = cnMF.rfc3339(endDate,true);
		debug.debug(" getGCalData(): start-max: "+startmax);
		//debug.log(' getGCalData(): start-max: '+startmax);
		//console.log( "startmax ready!", startmax );
	...
	function parseGCalData (gCalUrl, cdata, startDate, endDate, callbacks ) {
		...
		//console.log("1st cdata.feed.title", cdata.feed.title);
		calendarInfo.gCalUrl = gCalUrl;
		calendarInfo.gcTitle = cdata.feed.title ? cdata.feed.title['$t'] : 'title unknown';
		//console.log("calendarInfo.gcTitle", calendarInfo.gcTitle);
		calendarInfo.gcTitle = cdata.feed.title ? cdata.feed.title['$t'] : 'title unknown';
		//console.log("calendarInfo.gcTitle", calendarInfo.gcTitle);
		calendarInfo.gcTitle.replace(/"/,'&quot;');
		calendarInfo.gcLink = cdata.feed.link ? cdata.feed.link[0]['href'] : '';
		calendarInfo.desc = cdata.feed.subtitle ? cdata.feed.subtitle['$t'] : 'subtitle unknown';
		calendarInfo.gccolorId = cdata.feed.ColorId ? cdata.feed.ColorId['$t'] : 'ColorId unknown';
		...
		if (!cnMF.tz.computedFromBrowser) {
			...
			//debug.log(" Displaying calendar times using calendar timezone: "+ cnMF.tz.name);
		}

		for (var ii=0; cdata.feed.entry && cdata.feed.entry[ii]; ii++) {
			var curEntry = cdata.feed.entry[ii];
				var calname = curEntry;
				var calendartitle = curEntry.title;
				var calendarname = curEntry.name;
				//console.log( "calendartitle and calendarname", calendartitle,calendarname );
			...
			if (!(curEntry['gd$when'] && curEntry['gd$when'][0]['startTime'])) {
				...
				//debug.log(" skipping cal curEntry (no gd$when) %s (%o)", curEntry['title']['$t'], curEntry);
				continue;
			};
			var url = {};
			console.log("fsdfadfdffd curEntry "+ii+": ", cdata.feed.entry[ii]);
			for (var jj=0; curEntry.link[jj]; jj++) {
				var curLink = curEntry.link[jj];
				 //if a_link.rel == 'attendee' :
				if (curLink.rel == 'alternate') {
					...
					//var colorId[curLink.rel] = curLink.attendees.displayName;
					//colorId[curLink.rel] = curLink.colorId;
					console.log("curLink ", curLink);
				}
				
			}
			//var list = curEntry[ii];
			
			 // if (guests != "") {
				
				//guestmail = guests.attendees[0].email;
			 // } else {
			//	guestmail = '';
			//  }
			kk = cnMF.addEvent({
				...
				//var colorId  = curEntry.attendees;
				//dateStart: cnMF.parseDate(curEntry['gd$when'][0]['startTime']),
				//dateStart: "2013-06-13T23:00:00Z",
				//dateEnd: "2016-06-14T03:00:00Z",
				dateStart: cnMF.parseDate(curEntry['gd$when'][0]['startTime']),
				dateEnd: cnMF.parseDate(curEntry['gd$when'][0]['endTime']),
				colorId: curEntry['gd$who'][0]['attendeeType'] || '',
				title: calendarInfo.gcTitle,
				image: image 
			});
  ...
  */
			...
			//console.log("parsed curEntry "+ii+": ", kk.name,kk.colorId,kk.title, curEntry);
			
			//console.log("1st kk.title,kk.colorId ", kk.title,kk.colId);
		}
	...
	//console.log("2nd this.title,image ", this.title,image);	
	...
	MarkerClass.prototype.init = function ( eventObj, gMap, calTitle,title,colId) {
		this.colorId = [eventObj.colId];	
		this.title = [eventObj.name];	
		this.image = [eventObj.image];
		this.coordsStr = eventObj.getCoordsStr();
		this.eventList = [eventObj.id];
		this.gCalid = [eventObj.eventid];
		var str1 = this.title;
		//var lat = eventObj.getAttribute("eventid");
		//var x = myFunction(this.title);
		
		//console.log("3rd this.title,eventObj.colorId ", this.title,eventObj.colorId);
		var customIcons = {
		  purple: {
			icon: './img/CheersMemoriesLogoTestSm.png'
		  },
		  shmik: {
			icon: './img/shmikmapicon.png'
		  }
		};
		
		var image;
		if (eventObj.name.substring(0,1) == "A") { //Attraction
			//alert ("this.title", this.title);
		  image = 	'./img/themepark.png';
		} else if (eventObj.name.substring(0,1) == "M") { //Motel
		  image = 	'./img/motel.png';  
		} else if (eventObj.name.substring(0,1) == "C") {  //Car Rental
		  image = 	'./img/carrental.png';  
		} else if (eventObj.name.substring(0,1) == "R") { //Restaurant
		  image = 	'./img/restaurant.png';  
		} else if (eventObj.name.substring(0,1) == "L") { //Location
		  image = 	'./img/landmark.png';  
		} else if (eventObj.name.substring(0,1) == "T") { //Travel
		  image = 	'./img/air.png';  
		} else if (eventObj.name.substring(0,1) == "S") { //Sightseeing
		  image = 	'./img/sightseeing.png';  
		} else if (eventObj.name.substring(0,1) == "D") { //Driving
		  image = 	'./img/car.png';  
		} else if (eventObj.name.substring(0,1) == "B") { //Buying
		  image = 	'./img/shopping.png';  
		} else if (eventObj.name.substring(0,1) == "?") { //split
		  image = 	'./img/split.png';  
		}  else if (eventObj.name.substring(0,1) == "Z") { //anniversary
		  image = 	'./img/anniversary.png';  
		} else  {
		  image = 	'./img/pin.png';  //all others
		}
					/*var image = {
										if (this.title.substring(0,1) == 'A') {
											'./img/CheersMemoriesLogoTestSm.png',
											 size: new google.maps.Size(25, 34),
											origin: new google.maps.Point(0, 0)
											} else {
											'./img/shmikmapicon.png',
											 size: new google.maps.Size(25, 34),
											origin: new google.maps.Point(0, 0)
											}
	...
	MarkerClass.prototype.getEvents = function(){
		...
		//console.log("this.eventList ", this.eventList); //we dont see this
	}
		
	MarkerClass.prototype.getEvent = function(markerEventIndex){
		return this.eventList[markerEventIndex];
		//console.log("this.eventList[markerEventIndex] ", this.eventList[markerEventIndex]);//we dont see this
	}
	...
	window.cnMF = cnMF;
	//console.log( "window.cnMF and cnMF are the same objects - ready!", typeof(window.cnMF),window.cnMF,typeof(cnMF), cnMF );
})();
