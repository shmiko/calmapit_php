...
$(document).ready(function() {
	...
	*/
	...
	var myURL = parseURL(window.location.href); /* Call to function parseURL(url) */
	...
	function parseURL(url) {
		...
		a.href = url;
		console.log( "in the parse url function! - return a - which is the url", a );
		return {
			source: url,
		...
		console.log( "end of parse url function! - return ret", ret );  /*why don't we see this? */
	}
	...
	function httpGetParam( name ) {
		...
		console.log("httpGetParam()", name, results); /* this returns NULL for everything - gets called 8 times for cd,lat,lng,z,m,z,sd,ed */
	...
	function validateFloat(httpParamName, min, max, def) {
			...
			value = parseFloat(value); /*  Parses a string and returns a floating point number  */
			if (isNaN(value)) /*//The isNaN() function determines whether a value is an illegal number (Not-a-Number).This function returns true if the value is NaN, and false if not.*/
				value = def;
			if (value < min)
				value = min;
			if (max < value)
	...
	cnMFUI.init(opts); //'This is a function and returns var opts above
	//console.log("cnMFUI", typeof (cnMFUI), cnMFUI); //' seems to be returning object has such things as defaults, htmldecode,htmlencode,init,jumptoaddress,maxstr and opts
	//console.log("cnMFUI.init", typeof (cnMFUI.init), cnMFUI.init); //' seems to be returning the actual function itself
}); // end document ready

(function($) { /*function literal it ececutes immediatley and returns to parent*/

	var startMs2 = (new Date("March 10, 2016 00:00:00")).getTime();
	console.log('????????????????????startMs2', startMs2); //this never show up a result
	// google variables
	var myGmap;
   ...
   */
	function cnMFUI_init (options) {
		...
		var mapClickListener = false,
			...
			calendarsDecoding,
			emptyTableHtml = "<td>data</td><td>...</td><td>is<td><td>now</td><td>loading...</td><td>be Patient!</td>";

		/*
		 *   static variables
		 */
		//var
		//urlIconDefault = "http://www.google.com/mapfiles/marker.png", //images/CheersMemoriesLogoTestSm.png
		urlIconDefault = "./img/CheersMemoriesLogoTestSm.png", //images/CheersMemoriesLogoTestSm.png
		...
		mapVersion = "Map Version 2014-4-8";
		...
		function init() {
			...
			console.log( "calendarURLs ready!",calendarURLs );
		...
		function getCalendarURLs() {
			...
			calendarURLs.push("https://www.google.com/calendar/feeds/15dcnca6hga2rqna9f651qc5d0@group.calendar.google.com/public/full");
			calendarURLs.push("https://www.google.com/calendar/feeds/3c18dcosa63dql7l2gj1l7akt8@group.calendar.google.com/public/full");
		...
		function initDivs() {
			...
			$('#gcmLogo').html("<h3><a href='http://shmik.com/' class='jumpLink' target='_blank' title='open SHMIK in new window'>SHMIK</a></h3>");
		...
		function initGMap() {
			...
			var myOptions = {
				...
				center: new google.maps.LatLng(35.496456,-101.981445),
			...
			var FusionTablesLayer = new google.maps.FusionTablesLayer({
			  ...
			  query: {
				select: "col3",
				from: "1HyZwIoiwZ58r7RP1T8oIjoIulN6tKRM8YMOXbbvf",
				where: ""
			  },
			  options: {
				styleId: 2,
			...
			FusionTablesLayer.setMap(myGmap);	

			
			myGmap.mapTypes.set(MY_MAPTYPE_ID, customMapType);
			myGmap.setMapTypeId(MY_MAPTYPE_ID);
		...
		function date2days(date) {
		 	...
		 	return 0;
			//debug.log('date2days('+date+') invalid date ready!', date );
		}

		function days2date(nDays) {
			var day = new Date();
			//debug.log('day ready!', day );
		...
		function updateResults() {
			...
			//debug.log('updateResults() cnMF.countUnknownAddresses:', cnMF.countUnknownAddresses)
		...
		function createResultsTable(divId){
			...
			tableHtml = "<table id='"+divId+"Table' class='tablesorter'><thead><tr>"
					...
					+ "<th title='Click to Sort by Event Type'>Event Type</th>"
		...
		function updateResultsTable(divId, onlyValidCoords, clearTable){ // called 824,826
			var rowHTML = '';
			for (var i in cnMF.eventList) {
				...
				var typeofevent;
				if (kk.name.substring(0,1) == "A") { //Attraction
					//alert ("this.title", this.title);
				  typeofevent = 	'Attraction';
				} else if (kk.name.substring(0,1) == "M") { //Motel
				  typeofevent = 	'Motel';  
				} else if (kk.name.substring(0,1) == "C") {  //Car Rental
				  typeofevent = 	'Car Rental';  
				} else if (kk.name.substring(0,1) == "R") { //Restaurant
				  typeofevent = 	'Restaurant';  
				} else if (kk.name.substring(0,1) == "L") { //Location
				  typeofevent = 	'Location';  
				} else if (kk.name.substring(0,1) == "T") { //Travel
				  typeofevent = 	'Travel';  
				} else if (kk.name.substring(0,1) == "S") { //Sightseeing
				  typeofevent = 	'Sightseeing';  
				} else if (kk.name.substring(0,1) == "D") { //Driving
				  typeofevent = 	'Driving';  
				} else if (kk.name.substring(0,1) == "B") { //Buying
				  typeofevent = 	'Buying';  
				} else if (kk.name.substring(0,1) == "?") { //split
				  typeofevent = 	'Not Sure';  
				}  else if (kk.name.substring(0,1) == "Z") { //anniversary
				  typeofevent = 	'20th Anniversary';  
				} else  {
				  typeofevent = 	'Misc';  //all others
				}
				...
				if (str.indexOf("|") != -1) {
						rowHTML += "<td>[<a href='" + kk.url + "' class='jumpLink' title='Click to view Calendar Event Web Page in new window'"
							...
							+ cnMFUI.maxStr(kk.desc.substring(str.indexOf("|")+8), 140, 0, '', 1) + "</td>";
							//+ str.indexOf("|")+"---"+ str.substring(str.indexOf("|")+10)+"</td>";
				} else  {
						rowHTML += "<td>[<a href='" + kk.url + "' class='jumpLink' title='Click to view Calendar Event Web Page in new window'"
							+" target='_blank'>Event Details</a>] "
							+ cnMFUI.maxStr(str.substring(200), 140, 0, '', 1) + "</td>";
							//+ kk.desc.substring(kk.desc.indexOf('|')+1, 140) + "</td>";
				}
				...
				rowHTML += "</tr>\n";
				//console.log("kk.desc", cnMFUI.maxStr(kk.desc.substring(kk.desc.indexOf('|')+1, 10), 10, 0, '', 1) );
		...
		*/
		...
		function getMapType(oMap) { //called
			return -1;
		...
		//Click to show/hide control panel
		function mapRightTab(mapId) {
			$('#'+mapId).append("<a id='rightTab' title='Click to show/hide control panel'>-</a>");
			$('#rightTab').click(function(){
				debug.info('--- clicked tab, '+ (drawerIsOpen() ? 'clos':'open') + 'ing control panel');
		...
		function buildInfoHtml (eventObj) {
			...
			infoHtml += ' title="Get Directions using maps.google.com">Directions</a><br>';
			infoHtml += '<a href="' + eventObj.url + '" class="actionable" target="_blank"';
			infoHtml += ' title="Click to edit Calendar">Edit Event Details</a>';
			infoHtml += '</div></div>';
			return infoHtml;
			//console.log("infoHtml kk.gCalId", eventObj.colorId);
		}
		...
		function eventClicked(eventObj) {
			console.log("--- eventClicked", eventObj.id, eventObj);
			//console.log("eventClicked", eventObj.id, eventObj.colorId,eventObj);
...
*/
	 ...
	 */
		...
		function setupChangeDates() {
			  var html = ''
				 ...
				 + 'New end date: <input type="text" id="endDate" size="10" value="'+days2date(cnMF.origEndDay)+'"> '
			  	 + '<p>Reloading will fetch all calendar event data between start and end date, then reload map.  This is needed if you want to expand the date range.'
		...
		function cbCalendarLoad(calendarInfo) {
			...
			if (cnMF.validCalendars) {
				cnMF.validCalendars++;
				document.title = "Shmik "+ cnMF.validCalendars + " valid calendars";
				...
				document.title = "Shmik - "+ calendarInfo.gcTitle;
  ...
  window.cnMFUI = {
	...
	//console.log("window.cnMFUI", typeof (window.cnMFUI), window.cnMFUI); //' seems to be returning default data
	//object
	defaults:  {
		startDay: -2,
		endDay: 872,
		jumpTxt: "Jump to City, Address, or Postcode",
		...
		mapCenterLt: '35.496456',
		mapCenterLg: '-101.981445',
		mapZoom: 6,
		mapType: 0,
		mapAllOnInit: true,
