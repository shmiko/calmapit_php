		...
		$.ajax({
			...
			success:function(data) {
				$.each(data.feed.entry, function(e, item) {
					s+='<div class="entry">';
					s+='<h3>'+formatDate(item.gd$when[0].startTime,def.DateFormat.trim())+'</h3>';
					...
					item.gd$where[0].valueString?s+='<p class="location"><span>Location: </span><span>'+item.gd$where[0].valueString+'</span></p>':null;
					s+='</div>';
				});
				$('#'+id).append(s);	
			},
