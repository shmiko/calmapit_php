...
class SimpleImage {
   ...
   function resize($width, $height){
	...
	// The methods added by Dong Nguyen to take this better
	...
	function fillTo($width, $height){
      $ow = $this->getWidth();
      $oh = $this->getheight();
	   $size = $this->normalize($ow, $oh, $width, $height, 'fill');
      $new_image = imagecreatetruecolor($width, $height);
      imagecopyresampled($new_image, $this->image, $size['l'], $size['t'], 0, 0, $size['w'], $size['h'], $this->getWidth(), $this->getHeight());
      $this->image = $new_image;	  
	}
	function fitTo($width, $height){
      $ow = $this->getWidth();
      $oh = $this->getheight();
	   $size = $this->normalize($ow, $oh, $width, $height, 'fit');
      $new_image = imagecreatetruecolor($width, $height);
      imagecopyresampled($new_image, $this->image, $size['l'], $size['t'], 0, 0, $size['w'], $size['h'], $this->getWidth(), $this->getHeight());
