...
var MapsLib = {
  ...
  fusionTableId: "1VnXlhSBM_FglBZSnM7UZUWSyi4ZYg29Sg7Wh7pv4", //main table for building data
  
  povertyTableId: "1VnXlhSBM_FglBZSnM7UZUWSyi4ZYg29Sg7Wh7pv4",
  unemploymentTableId: "1VnXlhSBM_FglBZSnM7UZUWSyi4ZYg29Sg7Wh7pv4",
  populationTableId: "1VnXlhSBM_FglBZSnM7UZUWSyi4ZYg29Sg7Wh7pv4",
  medianIncomeId: "1VnXlhSBM_FglBZSnM7UZUWSyi4ZYg29Sg7Wh7pv4",

  violationsId: "1VnXlhSBM_FglBZSnM7UZUWSyi4ZYg29Sg7Wh7pv4",

  //*New Fusion Tables Requirement* API key. found at https://code.google.com/apis/console/
  //*Important* this key is for demonstration purposes. please register your own.
  googleApiKey:       "AIzaSyCDyuMEpvjNHZS8ACf1rJPhxMOODrfJyL4",

  //name of the location column in your Fusion Table.
  //NOTE: if your location column name has spaces in it, surround it with single quotes
  //example: locationColumn:     "'my location'",
  locationColumn:     "Event Location",

  map_centroid:       new google.maps.LatLng(38.250768035056424, -91.72011112031248), //center that your map defaults to
  locationScope:      "usa",      //geographical area appended to all address searches
  recordName:         "title",       //for showing number of results
  recordNamePlural:   "description",

  searchRadius:       1610,            //in meters ~ 1/2 mile
  defaultZoom:        6,             //zoom level when map is loaded (bigger is more zoomed in)
  addrMarkerImage:    'image',//'http://chicagobuildings.org/images/blue-pushpin.png',
  ...
  initialize: function() {
   ...
   // $("#txtSearchAddress").val(MapsLib.convertToPlainString($.address.parameter('address')));
    ...
    var minDate = moment("Mar 1 2016"); // Jan 1st 2010
    var maxDate = moment("May 1 2016"); //now

    //starting values
    var startDate = moment("Mar 10 2016");//.subtract('months', 3); //past 3 months
    var endDate = moment("Apr 27 2016"); //now
    ...
    MapsLib.poverty = new google.maps.FusionTablesLayer({
      query: {from:   MapsLib.povertyTableId, select: "Event Location"}
    });
    MapsLib.unemployment = new google.maps.FusionTablesLayer({
      query: {from:   MapsLib.unemploymentTableId, select: "Event Location"}
    });
    MapsLib.population = new google.maps.FusionTablesLayer({
      query: {from:   MapsLib.populationTableId, select: "Event Location"}
    });
    MapsLib.medianIncome = new google.maps.FusionTablesLayer({
      query: {from:   MapsLib.medianIncomeId, select: "Event Location"}
    });
  ...
  doSearch: function() {
    ...
    var whereClause = "";//MapsLib.locationColumn;// + " not equal to ''";
	 whereClause += " 'Event Start' >= '" + $('#startDate').html() + "'";
      whereClause += " AND 'Event End' <= '" + $('#endDate').html() + "'";
	  console.log(" 1st whereClause: " + whereClause,address);
    if (MapsLib.violationsMode) {
     ...
     // whereClause += " AND " + searchViolations.slice(0, searchViolations.length - 1) + ")";
		...
		//console.log(" 2nd whereClause: " + whereClause);
  ...
  query: function(selectColumns, whereClause, callback) {
   ...
   // queryStr.push(" WHERE " + whereClause);
	var queryStr = "SELECT * FROM "+MapsLib.fusionTableId+" WHERE " + whereClause +"";
    var sql = encodeURIComponent(queryStr);
    $.ajax({url: "https://www.googleapis.com/fusiontables/v1/query?sql="+sql+"&callback="+callback+"&key="+MapsLib.googleApiKey, dataType: "jsonp"});
	console.log(" 3rdd queryStr: ",queryStr);
  },

  handleError: function(json,selectColumns) {
    if (json["error"] != undefined) {
		console.log(" 3rdd json: ",json);
      var error = json["error"]["errors"]
      console.log("Error in Fusion Table call!");
      for (var row in error) {
		
        console.log(" Domain: " + error[row]["domain"] + selectColumns);
  ...
  displayCount: function(whereClause) {
	...
	console.log(" 3rdd whereClause: ",whereClause);
  },

  displaySearchCount: function(json,selectColumns) {
    MapsLib.handleError(json,selectColumns);
    var numRows = 0;
    if (json["rows"] != null)
      numRows = json["rows"][0];
	  console.log(" 3rdd numRows: ",numRows);	
